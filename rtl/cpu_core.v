...
module cpu_core #(
    ...
    wire [XLEN-1:0] load_resp_data = format_load_data(load_resp_funct3, dmem_rdata);
    (* keep = "true" *) wire [XLEN-1:0] load_resp_forward_data = format_load_data(load_resp_funct3, dmem_rdata);
    ...
    multiplier #(
        ...
        .a_i(id_ex_word_op ? sign_extend_word(forward_a_data[31:0]) : forward_a_data),
        .b_i(id_ex_word_op ? sign_extend_word(forward_b_data[31:0]) : forward_b_data),
        .busy_o(mul_busy),
        .valid_o(mul_valid),
        .result_o(mul_result)
    );
    ...
    generate
        if (FAST_MUL != 0) begin : gen_fast_mul_result
            wire [XLEN-1:0] fast_mul_a = id_ex_word_op ? sign_extend_word(forward_a_data[31:0]) : forward_a_data;
            wire [XLEN-1:0] fast_mul_b = id_ex_word_op ? sign_extend_word(forward_b_data[31:0]) : forward_b_data;
            wire signed [(2*XLEN)-1:0] fast_mul_product_ss = $signed(fast_mul_a) * $signed(fast_mul_b);
            wire [(2*XLEN)-1:0] fast_mul_product_uu = fast_mul_a * fast_mul_b;
            wire signed [(2*XLEN):0] fast_mul_product_su = $signed({fast_mul_a[XLEN-1], fast_mul_a}) *
                                                           $signed({1'b0, fast_mul_b});
    ...
    wire id_load_early_read = (ENABLE_ID_LOAD_EARLY_READ != 0) &&
                              ...
                              !dmem_port_busy &&
                              !ctrl_load_pending_valid;
    wire [XLEN-1:0] id_load_early_base_data = (if_id_load_rs1_q == 5'd0) ? {XLEN{1'b0}} :
                                          if_id_rs1_raw_data_q;
            ...
            if (flush) begin
                ...
                div_cmd_rs1_data <= id_ex_word_op ?
                                    (((id_ex_funct3 == 3'b100) || (id_ex_funct3 == 3'b110)) ?
                                     sign_extend_word(forward_a_data[31:0]) :
                                     zero_extend_word(forward_a_data[31:0])) :
                                    forward_a_data;
                div_cmd_rs2_data <= id_ex_word_op ?
                                    (((id_ex_funct3 == 3'b100) || (id_ex_funct3 == 3'b110)) ?
                                     sign_extend_word(forward_b_data[31:0]) :
                                     zero_extend_word(forward_b_data[31:0])) :
                                    forward_b_data;
