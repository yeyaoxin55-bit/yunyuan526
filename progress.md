...
## 2026-05-27 CSR Branch Session
- User goal: work on branch `新增CSR` to improve CSR functionality and make the RV32IM CPU closer to industrial quality.
- Confirmed current branch is `新增CSR`; initial `git status --short` was clean.
- Restored existing planning context from `task_plan.md`, `findings.md`, and `progress.md`; prior accepted board/performance baseline remains timing-fragile, so CSR work should be staged and verified.
- Explored CSR-related RTL/tests/scripts: `rtl/csr_unit.v`, `rtl/cpu_core.v`, `rtl/decoder.v`, `rtl/exception.v`, `rtl/hazard_unit.v`, `tb/tb_csr_counter.v`, `scripts/run_modelsim.ps1`, `scripts/run_riscv_test.ps1`, `scripts/run_riscv_suite.ps1`, and local riscv-test environment files.
- No RTL implementation has been started yet. Current phase is design clarification per brainstorming workflow.
- User added a hard parameterization requirement: the CSR/trap design should support both RV32 and RV64, not bake in 32-bit CSR state or constants.
- Read current `cpu_core.v` redirect/flush/store/predictor paths and official RISC-V Zicsr/M-machine CSR docs to shape the exception-priority design section.
- User provided a refined exception-priority and pipeline-interaction design. It keeps first-stage scope to M-mode synchronous exceptions and Zicsr, excludes asynchronous interrupt response, requires XLEN parameterization, and defines kill gates for register/memory/CSR/predictor/RAS/M-extension/replay/minstret side effects.
- Continuing design with CSR register table, WARL/WIRI policy, Zicsr read-modify-write behavior, and RV32/RV64 parameterization rules.
- Added the verification design direction: TDD, CSR unit tests first, then CPU integration tests, then official regression/smoke runs.
- Wrote implementation plan `docs/superpowers/plans/2026-05-27-machine-mode-csr.md` from the approved spec. The plan is task-based, test-first, and staged from CSR unit tests to CPU trap integration and regression.
- Started subagent-driven execution. Dispatched Task 1 (`Zicsr Unit Test and CSR Bank Core`) to worker subagent Faraday with ownership of `rtl/defines.vh`, `rtl/csr_unit.v`, `tb/tb_csr_unit_zicsr.v`, `scripts/run_csr_unit_modelsim.ps1`, and `scripts/run_modelsim.ps1`.
- Task 1 worker completed commit `cc6c142` with focused CSR test pass and full ModelSim reported pass. Spec compliance review passed via subagent Meitner. Multiple code-quality review subagents timed out; after closing the stuck agents, a main-session blocker-level quality review plus fresh `scripts/run_csr_unit_modelsim.ps1` run found no Critical/Important blocker. Focused run result: `PASS csr_unit zicsr regression completed`, 0 errors, 0 warnings.
- Initial Task 2 worker stalled and was closed. Split Task 2. Task 2a worker Socrates added trap/MRET and XLEN64 CSR unit tests plus script expansion in commit `ff49d39`. RED result: `tb_csr_unit_trap_mret` fails with `mstatus expected 00001880, got 00000008`; `tb_csr_unit_zicsr` passes; `tb_csr_unit_xlen64` already passes due to Task 1 RV64-capable CSR skeleton.
- Continued Task 2 CSR bank review. Fixed two spec gaps in commit `dabc472`: ordinary `mstatus` writes now force M-only `MPP=M`, and ordinary `mtvec` writes preserve MODE bit 0 while `trap_pc_o` still uses direct BASE. Focused CSR unit regression `scripts/run_csr_unit_modelsim.ps1` passed for zicsr, trap/mret, and XLEN64 with 0 errors and 0 warnings.
- Task 2 spec re-review found two remaining gaps: RV64 trap/MRET coverage was too shallow, and RV32 `mcycleh/minstreth` were supported but read as zero and had no write path. Added failing counter-high coverage (`mcycleh expected 12345678, got 00000000`), added RV64 trap/mret/mtval checks, then fixed `csr_unit` with 64-bit internal `mcycle/minstret` state and RV32 high-half read/write support in commit `25e28a0`. Verification: `scripts/run_csr_unit_modelsim.ps1` passed all three CSR unit tests with 0 errors/0 warnings; `scripts/run_modelsim.ps1` completed with exit 0, keeping the known temporary `cpu_core` unconnected CSR-port warnings until CPU integration tasks.
- A later Task 2 spec/quality review found reserved `mtvec.MODE=3` was WARL-converted to vectored instead of direct, trap/MRET/normal CSR update priority was not encoded as a single priority chain, and explicit counter writes could still be polluted by same-cycle implicit counter increments. Added RED tests for `mtvec.MODE=3`, trap/CSR and MRET/CSR same-cycle priority, `mcycle` low-half rollover, and `minstret` write-with-retire priority. Fixed these in commit `b744d30`. Verification: `scripts/run_csr_unit_modelsim.ps1` passed all three CSR unit tests with 0 errors/0 warnings; fresh `scripts/run_modelsim.ps1` completed with exit 0 and only the known temporary CSR-port warnings from `cpu_core`.
- Final Task 2 spec review returned `SPEC_COMPLIANT`. Final quality review found one remaining commit-side feature-gating issue: `SUPPORT_ZICSR=0` read was illegal but a direct commit could still update CSR state. Added a disabled-Zicsr commit test that failed with `disabled Zicsr read illegal=1 data=aaaaaaaa`, then gated CSR commit writes by `SUPPORT_M && SUPPORT_ZICSR && csr_op_legal` in commit `db4e2c8`. Verification: `scripts/run_csr_unit_modelsim.ps1` passed all three CSR unit tests with 0 errors/0 warnings; fresh `scripts/run_modelsim.ps1` completed with exit 0, keeping only the known temporary CSR-port warnings from `cpu_core`.
- Task 3 decoder system classification completed in commit `8c624f3`: `decoder` now emits `csr_op`, `sys_event`, and `illegal_instr`; `cpu_core` wires these decoder outputs for later pipeline tasks; `tb_decoder_system` is included in full ModelSim. RED was missing decoder ports `csr_op/sys_event/illegal_instr`; GREEN focused decoder test passed, and full `scripts/run_modelsim.ps1` completed with exit 0.
- Task 4 CPU normal CSR read/write integration completed: added bare-metal `csr_rw` program and `scripts/run_csr_trap_programs.ps1`, then connected `cpu_core` to the full `csr_unit` read/commit interface. CSR read data now comes from `csr_unit`; CSR writes commit from EX/MEM when not killed by replay flush; CSR immediate/register source data is captured for commit; a conservative CSR-after-CSR decode stall prevents a younger CSR read from seeing an older CSR write before commit. RED `csr_rw` originally failed with a fail marker at cycle 43; GREEN `scripts/run_csr_trap_programs.ps1 -Tests csr_rw` now passes with `PASS external program completed cycle=68`. Verification also passed for `scripts/run_csr_unit_modelsim.ps1` and full `scripts/run_modelsim.ps1`; full compile now has 0 CSR port warnings.
- Task 5 ECALL/MRET integration completed: added `sw/csr_trap_tests/ecall_mret.S`, pipelined system-event and original-instruction metadata into ID/EX, connected trap entry and MRET restore requests from `cpu_core` into `csr_unit`, added CSR redirect classification, and gave trap/MRET redirect priority over ordinary branch/jump redirect. The CPU now blocks the faulting trap/MRET instruction from reaching EX/MEM retire side effects, flushes younger frontend state on the registered CSR redirect, and conservatively stalls system events behind an older CSR write so `csrw mtvec; ecall` and `csrw mepc; mret` see committed CSR state. RED `ecall_mret` failed with a fail marker at cycle 44; GREEN `scripts/run_csr_trap_programs.ps1 -Tests csr_rw,ecall_mret` passed with `csr_rw` cycle 68 and `ecall_mret` cycle 69. Verification also passed for `scripts/run_csr_unit_modelsim.ps1` and full `scripts/run_modelsim.ps1`, both with 0 errors and 0 warnings.
- Task 6 illegal CSR and misaligned memory traps completed: added `illegal_csr`, `misaligned_store`, and extra `misaligned_load` bare-metal programs. `illegal_csr` was already green after Task 5's illegal-CSR trap path; `misaligned_store` RED failed with a fail marker at cycle 49. Added EX-stage effective-address alignment checks for halfword/word loads and stores, extended trap cause/mtval priority for causes 4 and 6, and gated reg/mem/CSR EX/MEM side effects with `id_ex_faulting`. GREEN `scripts/run_csr_trap_programs.ps1 -Tests csr_rw,ecall_mret,illegal_csr,misaligned_store,misaligned_load` passed: cycles 68, 69, 77, 72, and 73 respectively. Verification also passed for `scripts/run_csr_unit_modelsim.ps1` and full `scripts/run_modelsim.ps1`, both with 0 errors and 0 warnings.
- Task 7 first-stage trap coverage completed: added `ebreak`, general illegal-instruction, and branch/JAL/JALR instruction-address-misaligned bare-metal trap programs. RED checks exposed the missing general decoder illegal path and missing control-target misaligned trap path. `decoder` now marks unknown opcodes, invalid branch/load/store/JALR function fields, illegal shift-immediate encodings, and invalid OP funct7 combinations as illegal. `cpu_core` now detects taken branch/JAL/JALR targets with low bits set, traps with cause 0 and `mtval=bad_target`, uses the control instruction PC as `mepc`, and suppresses ID-stage early JAL/RAS redirect side effects for misaligned targets. The conservative CSR hazard was widened so a younger branch/jump/load/store trap observes an older CSR write before committing its own trap entry.
- Task 7 verification:
  - RED `scripts/run_csr_trap_programs.ps1 -Tests illegal_instr` failed with a fail marker at cycle 46 before decoder legality fixes; GREEN passed at cycle 75 in the full run.
  - RED `scripts/run_csr_trap_programs.ps1 -Tests misaligned_branch,misaligned_jal,misaligned_jalr` timed out before instruction-address-misaligned support; after RTL and test-label alignment fixes, GREEN passed at cycles 72, 76, and 78.
  - `ebreak` passed at cycle 71.
- First-stage CSR/trap regression on 2026-05-27:
  - `scripts/run_csr_trap_programs.ps1 -Tests csr_rw,ecall_mret,ebreak,illegal_csr,illegal_instr,misaligned_store,misaligned_load,misaligned_branch,misaligned_jal,misaligned_jalr`: pass, cycles 72, 72, 71, 80, 75, 74, 75, 72, 76, 78.
  - `scripts/check_project.ps1`: pass.
  - `scripts/run_csr_unit_modelsim.ps1`: pass for Zicsr, trap/MRET, and XLEN64 tests with 0 errors and 0 warnings.
  - Full `scripts/run_modelsim.ps1`: pass with 0 errors and 0 warnings.
  - Official `rv32ui` selected suite passed: add, addi, and, andi, auipc, branches, jal/jalr, loads/stores, shifts, set-less-than, sub, xor/or variants.
  - Official `rv32um` full suite passed: div, divu, mul, mulh, mulhsu, mulhu, rem, remu.
