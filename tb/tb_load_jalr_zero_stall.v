...
module tb_load_jalr_zero_stall;
    reg clk;
    reg rst;
    integer cycle;
    ...
    cpu_top #(
        ...
        .IMEM_INIT_FILE("tb/programs/load_jalr_strict.hex"),
        .DMEM_INIT_FILE("")
    ) dut (
        .clk(clk),
        .rst(rst),
        ...
        for (cycle = 0; cycle < 120; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word1 == 32'd1) begin
                if (debug_dmem_word3 != 32'd7) begin
                    $display("FAIL load-jalr-zero-stall result: word3=%08x", debug_dmem_word3);
                    $finish;
                end
                if (load_use_stall_count != 0) begin
                    $display("FAIL load-jalr-zero-stall count=%0d", load_use_stall_count);
                    $finish;
                end
                $display("PASS load-jalr zero-stall regression completed");
                $finish;
            end
        end

        $display("FAIL load-jalr-zero-stall timeout: word1=%08x word3=%08x count=%0d",
