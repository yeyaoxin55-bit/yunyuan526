...
module tb_soc_uart_reset_start;
    ...
    task fail;
        input [1023:0] msg;
        begin
            $display("FAIL soc_uart_reset_start: %0s", msg);
            $finish;
        end
    endtask
    ...
    initial begin
        ...
        if (over !== 1'b0 || succ !== 1'b0) begin
            fail("CPU ran while UART download key was active");
        end
        ...
        if (over !== 1'b0 || succ !== 1'b0) begin
            fail("START packet ran CPU before reset in UART download mode");
        end

        uart_debug_key_n = 1'b1;
        wait_clocks(80);
        if (over !== 1'b0 || succ !== 1'b0) begin
            fail("CPU ran after releasing download key without reset");
        end
        ...
        if (over !== 1'b1) fail("over did not assert after post-download reset");
        if (succ !== 1'b1) fail("succ did not assert after post-download reset");
        if (halted_ind !== 1'b0) fail("halted_ind is not tied low");

        $display("PASS soc_uart_reset_start completed");
        $finish;
    end
endmodule
