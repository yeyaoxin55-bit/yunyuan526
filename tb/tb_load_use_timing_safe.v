...
module tb_load_use_timing_safe;
    reg clk;
    reg rst;
    integer cycle;
    ...
    initial begin
        ...
        for (cycle = 0; cycle < 120; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word1 == 32'd1) begin
                ...
                $display("PASS load-use timing-safe regression completed");
                $finish;
            end
        end

        $display("FAIL load-use-timing-safe timeout: word1=%08x word2=%08x count=%0d",
