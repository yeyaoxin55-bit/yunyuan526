        ...
        for (cycle = 0; cycle < 100; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word1 == 32'd1) begin
                if (debug_dmem_word2 != 32'd43) begin
                    $display("FAIL load-use-one-stall result: word2=%08x", debug_dmem_word2);
                    $finish;
                end
                if (load_use_stall_count != 1) begin
                    $display("FAIL load-use-one-stall count=%0d", load_use_stall_count);
                    $finish;
                end
                $display("PASS load-use one-stall regression completed");
                $finish;
            end
        end

        $display("FAIL load-use-one-stall timeout: word1=%08x word2=%08x count=%0d",
