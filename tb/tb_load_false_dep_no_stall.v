...
module tb_load_false_dep_no_stall;
    reg clk;
    reg rst;
    integer cycle;
    ...
    cpu_top #(
        ...
        .IMEM_INIT_FILE("tb/programs/load_false_dep.hex"),
        .DMEM_INIT_FILE("")
    ) dut (
        .clk(clk),
        .rst(rst),
        ...
        for (cycle = 0; cycle < 80; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word1 == 32'd1) begin
                if (load_use_stall_count != 0) begin
                    $display("FAIL load-false-dep-no-stall count=%0d", load_use_stall_count);
                    $finish;
                end
                $display("PASS load false-dep no-stall regression completed");
                $finish;
            end
        end

        $display("FAIL load-false-dep-no-stall timeout: word1=%08x count=%0d",
                 debug_dmem_word1, load_use_stall_count);
        $finish;
    end
endmodule
