...
module tb_load_use_zero_stall_early_read;
    reg clk;
    reg rst;
    integer cycle;
    ...
    cpu_top #(
        ...
        .IMEM_INIT_FILE("tb/programs/load_use_early.hex"),
        .DMEM_INIT_FILE("")
    ) dut (
        .clk(clk),
        .rst(rst),
        ...
        for (cycle = 0; cycle < 100; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word1 == 32'd1) begin
                if (debug_dmem_word2 != 32'd43) begin
                    $display("FAIL load-use-zero-stall-early-read result: word2=%08x", debug_dmem_word2);
                    $finish;
                end
                if (load_use_stall_count != 0) begin
                    $display("FAIL load-use-zero-stall-early-read count=%0d", load_use_stall_count);
                    $finish;
                end
                $display("PASS load-use zero-stall early-read regression completed");
                $finish;
            end
        end

        $display("FAIL load-use-zero-stall-early-read timeout: word1=%08x word2=%08x count=%0d",
                 debug_dmem_word1, debug_dmem_word2, load_use_stall_count);
        $finish;
    end
endmodule
