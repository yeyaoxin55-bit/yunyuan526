...
module tb_mul_result_no_early_forward;
    reg clk;
    reg rst;
    integer cycle;
        ...
        for (cycle = 0; cycle < 100; cycle = cycle + 1) begin
            ...
            if (debug_dmem_word0 == 32'd8) begin
                if (mul_src_stalls <= 1) begin
                    $display("FAIL expected no-early-forward path to add a mul source stall, got %0d",
                        mul_src_stalls);
                    $finish;
                end
                $display("PASS multiply result without early forwarding completed");
                $finish;
            end
        end
