...
## 2026-06-05 Phase60B CPU Integration v2 Findings
- The conservative M-unit CPU boundary v2 was functionally viable: ModelSim passed, `rv32um` passed under `FAST_MUL=0 / MUL_STAGES=1`, and CSR phase acceptance passed with `CSR_PHASE_ACCEPTANCE_PASS=1`.
- Removing same-cycle M response forwarding into branch/JALR/control paths is acceptable for the short performance screen when the fence is narrowed. An over-conservative first fence failed the CoreMark screen at `683157` cycles; the narrowed trap/order fence passed at `664509` cycles, CPI `1.135969`, with `COREMARK_MUL_WAIT_STALLS=1947`.
- The physical run rejects the candidate decisively. Full Huoyue `soc_top` implementation with the same aggressive strategy as prior timing screens generated a bitstream and passed QoR, but WNS was `-2.718 ns`, much worse than the `-1.064 ns` retention artifact.
- The new worst path moved into the frontend prediction clear/enable cone: `u_core/ex_mem_rd_reg[4]/C` -> `u_core/u_prefetch/current_pred_target_reg[13]/CE`, data delay `12.404 ns`, route `80.628%`, logic levels `13`. This means the v2 scoreboard/epoch wiring still fans into a broad wrong-path/redirect control region even without same-cycle M response forwarding.
- Vivado DRC warnings on DSPs under `u_core/gen_m_unit_active.u_m_unit/gen_xilinx_backend.u_backend/product*` show that the inferred Xilinx backend still leaves DSP input/output pipeline registers below an industrial SoC integration standard.
- Decision: revert the Phase60B CPU integration v2 RTL/check/test changes. Keep Phase60A. The next attempt should not repeat request/response CPU integration alone; first build either a true DSP48 primitive/macro backend with explicit A/B/M/P pipeline control, or isolate the prefetch/current-prediction redirect CE boundary that this run exposed.
- Fresh post-revert CSR acceptance passed and CoreMark 2 returned to `649893` cycles, confirming the retained branch state is again the Phase60A backend/OOC foundation rather than an active CPU M-unit replacement.

## 2026-06-05 Phase61 Prefetch Valid-Only Flush Strategy Findings
- The Phase60B worst path ended at `u_core/u_prefetch/current_pred_target_reg[13]/CE`, while existing `rtl/prefetch.v` clears payload registers in the `flush_i` branch. This is a direct frontend payload clear path, not a CSR-bank or multiplier arithmetic path.
- The prefetch payload registers are already valid-tagged. `cpu_core` only consumes prefetch payloads as instructions when `prefetch_valid` is true; otherwise IF/ID captures NOP/default prediction values. Therefore flush should only need to clear `current_valid` and `skid_valid`.
- Phase61 is intentionally narrower than another redirect policy rewrite: it does not add a pending redirect packet, does not change predictor update policy, and does not retry M-unit CPU integration. It only removes unnecessary payload assignments from the frontend flush branch and verifies the semantic contract.
- The candidate must still be physically screened. A functional pass alone is not enough, because previous local RTL rewrites repeatedly preserved simulation while worsening Huoyue timing.
- The Phase61 valid-only flush candidate was functionally safe but physically rejected. Focused `tb_prefetch`, structural checks, full ModelSim, and CSR phase acceptance all passed; CoreMark 2 remained `649893` cycles.
...
- Phase62B will start with `writeback_arb.v` because it is a low-risk extraction with a concrete behavior contract. It must not own replay/flush/trap/MRET/redirect policy; those remain in `cpu_core`.
- The old shared writeback behavior gives load responses priority over multiply responses by using `load_wb_write_en` as the slot-occupation signal. The extracted arbiter must preserve this even when the load writes `x0`, because allowing a multiply response through in that case would change M retire timing.
- The extraction is still timing-sensitive enough to require Huoyue physical screening. Cleaner hierarchy alone is not a retention criterion on this branch.
