...
if {![file exists $post_route_dcp]} {
    puts "ERROR: Missing checkpoint '$post_route_dcp'"
    exit 1
}
...
write_checkpoint -force [file join $out_dir "post_route_physopt.dcp"]
report_utilization -file [file join $out_dir "utilization_post_route_physopt.rpt"]
report_timing_summary -file [file join $out_dir "timing_summary_post_route_physopt.rpt"]
report_clocks -file [file join $out_dir "clocks_post_route_physopt.rpt"]
if {[llength [info commands report_ram_utilization]] > 0} {
    report_ram_utilization -file [file join $out_dir "ram_utilization_post_route_physopt.rpt"]
}

set bitstream_file [file join $out_dir "${top}_physopt.bit"]
