param(
  ...
  [string]$XdcPath = "",
  [string]$OutDir = "build/vivado_fast_synth",
  [int]$Jobs = 4,
  [string[]]$Generic = @(),
...
Write-Host "Fast timing stage: post-synth"
Write-Host "Top: $Top"
...
if ($DryRun) {
  ...
  Write-Host "Vivado post-synth fast timing dry run completed"
  exit 0
}
...
$output
if ($LASTEXITCODE -ne 0) {
  throw "Vivado post-synth fast timing failed"
}

$timingReport = Join-Path $resolvedOutDir "timing_summary_post_synth.rpt"
$pathReport = Join-Path $resolvedOutDir "timing_paths_post_synth.rpt"
...
Write-Host "Vivado post-synth fast timing completed"
